...
`getFlats`, `getEnharmonicLabels`, and `getSharps` respell already-computed sharp `Note[]` output as flats, combined enharmonic labels, or back to canonical sharps - without ever changing the `Note` type itself. `parseNote` and `parseNoteToken` accept flat-spelled input (`Db`, `Eb`, `Gb`, `Ab`, `Bb`) too; see the [Keys & Modes](/theory/engine/) page.
