...
The scales module computes note sets across 7 scale types:

- `major`, `chromatic`, `pentatonic-major`, `pentatonic-minor`, `blues`, `harmonic-minor`
- `melodic-minor`

Each type is described by a `SCALE_DEFINITIONS` entry that records its label and how its notes are derived.
