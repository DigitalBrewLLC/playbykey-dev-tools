...
import { getParentScaleModes, MODE_IDS } from '../src';
...
const C_PARENT_INPUTS: Array<{
  ...
  { key: 'C', mode: MODE_IDS.IONIAN, degreeIndex: 0 },
  { key: 'D', mode: MODE_IDS.DORIAN, degreeIndex: 1 },
  { key: 'E', mode: MODE_IDS.PHRYGIAN, degreeIndex: 2 },
  { key: 'F', mode: MODE_IDS.LYDIAN, degreeIndex: 3 },
  { key: 'G', mode: MODE_IDS.MIXOLYDIAN, degreeIndex: 4 },
  { key: 'A', mode: MODE_IDS.AEOLIAN, degreeIndex: 5 },
  { key: 'B', mode: MODE_IDS.LOCRIAN, degreeIndex: 6 },
];
...
describe('getParentScaleModes', () => {
  describe('always returns exactly 7 mode pairs', () => {
    it('D dorian returns 7 pairs', () => {
      expect(getParentScaleModes('D', MODE_IDS.DORIAN)).toHaveLength(7);
    });

    it('B locrian returns 7 pairs', () => {
      expect(getParentScaleModes('B', MODE_IDS.LOCRIAN)).toHaveLength(7);
    });
  });

  describe('pairs are always ordered ionian → dorian → ... → locrian', () => {
    it('G mixolydian pairs have correct mode order', () => {
      const pairs = getParentScaleModes('G', MODE_IDS.MIXOLYDIAN);
  ...
  describe('all 7 modes of the C major parent key resolve correctly', () => {
    it.each(C_PARENT_INPUTS)(
      ...
      ({ key, mode, degreeIndex }) => {
        ...
        expect(pairs[0]).toEqual({ root: 'C', mode: MODE_IDS.IONIAN });
  ...
  describe('all 12 keys in dorian — correct parent Ionian root resolved for every transposition', () => {
    it.each(DORIAN_ALL_KEYS)(
      ...
      ({ key, parentIonian }) => {
        const pairs = getParentScaleModes(key, MODE_IDS.DORIAN);
        expect(pairs[0]?.root).toBe(parentIonian);
        expect(pairs[0]?.mode).toBe(MODE_IDS.IONIAN);
        expect(pairs[1]?.root).toBe(key);
        expect(pairs[1]?.mode).toBe(MODE_IDS.DORIAN);
      }
    );
  });
});
