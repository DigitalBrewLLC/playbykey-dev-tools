...
describe('getChordNotes', () => {
  it('returns a major triad', () => {
    expect(getChordNotes('C', ChordTypes.MajorTriad)).toEqual(['C', 'E', 'G']);
  });

  it('returns a minor 7th chord', () => {
    expect(getChordNotes('C', ChordTypes.Minor7th)).toEqual([
      'C',
      'D#',
      'G',
      'A#',
  ...
  it('returns a major 9th chord', () => {
    expect(getChordNotes('C', ChordTypes.Major9th)).toEqual([
      'C',
      'E',
      'G',
      'B',
  ...
  it('returns a sus4 chord', () => {
    expect(getChordNotes('C', ChordTypes.Sus4)).toEqual(['C', 'F', 'G']);
  });

  it('returns a major 13th chord (all 7 notes of the parent major scale)', () => {
    expect(getChordNotes('C', ChordTypes.Major13th)).toEqual([
      'C',
      'E',
      'G',
      'B',
...
describe('getAvailableInversions', () => {
  it('returns [0,1,2] for a triad type', () => {
    expect(getAvailableInversions(ChordTypes.MajorTriad)).toEqual([0, 1, 2]);
  });

  it('returns [0,1,2,3,4] for a 9th chord type', () => {
    expect(getAvailableInversions(ChordTypes.Major9th)).toEqual([
      0, 1, 2, 3, 4,
    ]);
  });

  it('returns [0,1,2,3,4,5,6] for a 13th chord type', () => {
    expect(getAvailableInversions(ChordTypes.Major13th)).toEqual([
      0, 1, 2, 3, 4, 5, 6,
    ]);
  });
});
...
describe('getChordInversion', () => {
  it('rotates a C major triad through all 3 inversions', () => {
    const chord = { root: 'C', type: ChordTypes.MajorTriad } as const;
  ...
  it('rotates a C dominant 7th chord (4 notes) through all 4 inversions', () => {
    const chord = { root: 'C', type: ChordTypes.Dominant7th } as const;
  ...
  it('rotates a C major 9th chord (5 notes) through all 5 inversions', () => {
    const chord = { root: 'C', type: ChordTypes.Major9th } as const;
  ...
  it('rotates a C major 13th chord (7 notes) through all 7 positions - the top of the widened 0-6 range', () => {
    const chord = { root: 'C', type: ChordTypes.Major13th } as const;
...
describe('detectChords', () => {
  ...
  it('finds every valid root for a symmetric diminished 7th chord', () => {
    expect(detectChords(getChordNotes('C', ChordTypes.Diminished7th))).toEqual({
      C: [ChordTypes.Diminished7th],
      'D#': [ChordTypes.Diminished7th],
      'F#': [ChordTypes.Diminished7th],
      A: [ChordTypes.Diminished7th],
    });
  });

  it('finds every valid root for a symmetric augmented triad', () => {
    expect(detectChords(getChordNotes('C', ChordTypes.AugmentedTriad))).toEqual(
      {
        C: [ChordTypes.AugmentedTriad],
        E: [ChordTypes.AugmentedTriad],
        'G#': [ChordTypes.AugmentedTriad],
      }
    );
  });

  it('finds a minor 7th chord and the major 6th chord sharing its notes at a different root', () => {
    expect(detectChords(getChordNotes('C', ChordTypes.Minor7th))).toEqual({
      C: [ChordTypes.Minor7th],
      'D#': [ChordTypes.Major6th],
    });
  });
});
